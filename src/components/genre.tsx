...
const Genre: React.FC<GenreData> = ({ genreList: initialGenreList }) => {
  ...
  const handleEditGenre = (genreId, currentName) => {
    setEditingGenreId(genreId); 
    setGenreName(currentName); 
    setTimeout(() => inputRef.current?.focus(), 0); 
  };

  const handleSaveGenre = async (genreId) => {
       if (!genreName.trim()) {
      toast({
        title: "Error",
        description: "Genre name cannot be empty.",
    ...
    try {
      ...
      if (response.success) {
        ...
        setGenreList((prevList) =>
          prevList.map((genre) =>
            genre.genreId === genreId ? { ...genre, name: genreName } : genre
          )
        );  
  ...
  const handleDeleteGenre = async (genreId) => {
    try {
      ...
      if (response.success) {
        ...
        setGenreList((prevList) =>
          prevList.filter((genre) => genre.genreId !== genreId)
        );
        setGenreName("");
      }
       else {
                  ...
                  >
                    ...
                    {isUpdating && (
                      <div className="flex gap-2">
                        {editingGenreId === genre.genreId ? (
                          <Button  onClick={() => handleSaveGenre(genre.genreId)} className="shrink-0 p-2">
                          <CheckIcon className="w-5 h-5 text-primaryColorPinkHover" />
                        </Button>
                        ) : (
                          <Button
