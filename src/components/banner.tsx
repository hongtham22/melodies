...
function Banner() {
  ...
  return (
    <div
      className={`${showContentSong ? 'w-[55vw]' : 'w-[75vw] h-[80vh]'} text-primaryColorPink my-4 px-6 py-4 rounded-3xl bg-cover bg-center flex gap-[114px] flex-col relative overflow-hidden`}
    >
      ...
      <div className="w-[352px] h-fit flex flex-col gap-[16px] justify-center">
        ...
        <div className={`${showContentSong ? 'gap-6' : 'justify-between'} flex p-2`}>
          <button className={`${showContentSong && 'text-[0.8rem]'} h-[40px] px-[24px] py-[8px] bg-primaryColorPink text-white rounded-sm hover:bg-darkPinkHover`}>Discover Now</button>
          <button className={`${showContentSong && 'text-[0.8rem]'} h-[40px] px-[24px] py-[8px] bg-transparent border border-primaryColorBlue text-primaryColorBlue rounded-sm`}>Create Playlist</button>
        </div>
      </div>
    </div>
